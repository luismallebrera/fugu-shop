...
/* Description */
$fugu_product_description_layout_meta = get_post_meta( $product->get_id(), 'fugu_product_description_full', true );
if ( $fugu_theme_options['product_description_layout'] == 'full' || ! empty( $fugu_product_description_layout_meta ) ) {
...
if ( ! empty( $product_tabs ) ) : ?>
    ...
    <div id="fugu-product-accordion">
        <?php
        foreach ( $product_tabs as $key => $product_tab ) :
            $panel_class = ( $key == 'description' ) ? ' entry-content' : '';
        ?>
        <div class="fugu-product-accordion-panel fugu-product-accordion-panel-<?php echo esc_attr( $key ); ?>">
            <a href="#panel-<?php echo esc_attr( $key ); ?>" class="fugu-product-accordion-heading" role="tab" aria-controls="tab-<?php echo esc_attr( $key ); ?>">
                <?php echo wp_kses_post( apply_filters( 'woocommerce_product_' . $key . '_tab_title', $product_tab['title'], $key ) ); ?>
            </a>

            <div class="fugu-product-accordion-content">
                <div class="fugu-product-accordion-content-inner<?php echo esc_attr( $panel_class ); ?>">
                <?php
    ...
    <div class="woocommerce-tabs wc-tabs-wrapper<?php echo esc_attr( $description_layout_class ); ?>">
        <div class="fugu-product-tabs-col">
            <div class="fugu-row">
                <div class="col-xs-12">
            ...
            ?>
                <div class="woocommerce-Tabs-panel woocommerce-Tabs-panel--<?php echo esc_attr( $key ); ?> panel wc-tab" id="tab-<?php echo esc_attr( $key ); ?>" role="tabpanel" aria-labelledby="tab-title-<?php echo esc_attr( $key ); ?>">
                    ...
                    <div class="fugu-row">
                        <div class="col-xs-12">
                            <div class="fugu-tabs-panel-inner<?php echo esc_attr( $tab_panel_class ); ?>">
                                <?php
                                if ( isset( $product_tab['callback'] ) ) { 
                                    call_user_func( $product_tab['callback'], $key, $product_tab );
                                }
