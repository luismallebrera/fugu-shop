...
$fugu_is_infinite_load = ( is_woocommerce() && $fugu_theme_options['shop_infinite_load'] !== '0' ) ? true : false;
?>
<p class="woocommerce-result-count" role="alert" aria-relevant="all" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'data-is-sorted-by="true"'; ?>>
	...
	if ( 1 === intval( $total ) ) {
        ...
        $first                 = ( $fugu_is_infinite_load ) ? 1 : ( $per_page * $current ) - $per_page + 1; // NM: Always show "1-X" when infinite load is enabled
