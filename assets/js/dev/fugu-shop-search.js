(function($) {	
	...
	$.extend($.nmTheme, {
		...
		search_init: function() {
            ...
            self.headerSearch = (fugu_wp_vars.shopSearchHeader !== '0') ? true : false;
            self.searchCurrentQuery = '';
            
            if (self.headerSearch) {
                // Suggestions
                if (fugu_wp_vars.searchSuggestions !== '0') {
                    self.instantSuggestions = (fugu_wp_vars.searchSuggestionsInstant !== '0') ? true : false;
                    ...
                    self.suggestionsMaxResults = fugu_wp_vars.searchSuggestionsMax;
                } else {
                    self.suggestions = false;
                }
                
                self.$searchPanel = $('#fugu-header-search');
                
                // Hide search-panel scrollbar
                //self.$searchPanel.css('margin-right', '-'+self.scrollbarWidth+'px');
                
                self.$searchBtn = $('#fugu-menu-search-btn');
                self.$searchInput = $('#fugu-header-search-input');
                self.$searchNotice = $('#fugu-header-search-notice');
                self.$searchKeywords = $('#fugu-search-keywords');
                
                self.headerSearchBind();
            }
            
            self.shopSearch = (! self.headerSearch && fugu_wp_vars.shopSearch !== '0') ? true : false;
            
            if (self.isShop && self.shopSearch) {
                self.$searchBtn = $('#fugu-shop-search-btn');
                self.$searchPanel = $('#fugu-shop-search');
                self.$searchInput = $('#fugu-shop-search-input');
                self.$searchNotice = $('#fugu-shop-search-notice');
		...
		searchValidateInput: function(searchKey, enterPress) {
			...
			/*ORG: if ((/\S/.test(searchKey)) && searchKey.length > (fugu_wp_vars.shopSearchMinChar - 1)) {
				return true;
			} else {
				return false;
			}*/
			...
			// Make sure the search string has at least one character (not just whitespace) and minimum allowed characters are entered
            if (searchQueryNoWhitespace.length > 0 && searchKey.length > (fugu_wp_vars.shopSearchMinChar - 1)) {
		...
		headerSearchBind: function() {
            ...
            $('#fugu-header-search-close').on('click', function(e) {
                e.preventDefault();
                self.headerSearchTogglePanel();
            });
            ...
            $('#fugu-header-search-clear-button').on('click', function(e) {
                e.preventDefault();
		...
		headerSearchTogglePanel: function() {
            ...
            if (self.suggestions && self.$body.hasClass(self.classSearchOpen)) {
                if ($('#fugu-search-suggestions-notice').hasClass('show')) { // Make sure suggestions are showing
                    self.suggestionsHide();
                    toggleDelay = 100;
                }
            }
            ...
            setTimeout(function() {
                if (! self.$body.hasClass(self.classSearchOpen)) {
                    ...
                    // "Focus" search input
                    $('#fugu-header-search-input').trigger('focus');
                    ...
                    setTimeout(function() {
                        ...
                        $('#fugu-header-search-input').val(''); // Empty input value
        ...
        headerSearchStaticSearch: function($input) {
            ...
            if (validSearch === 'valid') {
                ...
                if (self.suggestions) {
                    $('#fugu-header-search-form').addClass('fugu-loader');
                    $('.fugu-header-search-wrap').addClass('redirecting');
                }
                ...
                var searchUrl = fugu_wp_vars.shopSearchUrl.toString().replace('%%nmsearchkey%%', encodeURIComponent(searchKey));
		...
		shopSearchBind: function() {
            ...
            /* Bind: Panel "close" button */
            $('#fugu-shop-search-close').on('click', function(e) {
                e.preventDefault();
            ...
            /* Bind: Search input "input" event */
            $('#fugu-shop-search-input').on('input', function() {
                ...
                self.$searchInput.on('keypress', function(e) {
                    ...
                    if (keyCode == '13') {
                        ...
                        if (validSearch === 'valid' && self.currentSearch !== searchKey) {
                            ...
                            if ($input.hasClass('fugu-mobile-menu-search')) {
                                self.mobileMenuClose();
                                setTimeout(function() {
                                    $('#fugu-mobile-menu-shop-search-input').val(''); // Empty input value
                                    self.shopSearchAjaxSearch(searchKey);
                                }, self.panelsAnimSpeed);
                            } else {
                                if (fugu_wp_vars.shopSearchAutoClose != '0') {
                                    // Close search-panel
                                    self.$searchBtn.trigger('click');
                                } else {
                                    self.searchHideNotice();
		...
		shopSearchTogglePanel: function() {
			var self = this,
				$searchInput = $('#fugu-shop-search-input');
		...
		shopSearchAjaxSearch: function(searchKey) {
            ...
            // Create search URL
            var searchUrl = fugu_wp_vars.searchUrl.toString().replace('%%nmsearchkey%%', encodeURIComponent(searchKey)); // Note: Encoding the search string with "encodeURIComponent" to avoid breaking the AJAX url
        ...
        suggestionsGet: function(searchKey) {
            ...
            // Get form elements
            var $searchForm = $('#fugu-header-search-form'),
                $searchSuggestions = $('#fugu-search-suggestions');
            ...
            if (self.suggestionsCacheEnabled && self.suggestionsCache[cacheKey]) {
                ...
                $searchForm.addClass('fugu-loader');
                
                // Add class if suggestions are showing
                if ($searchSuggestions.children('.fugu-search-suggestions-inner').children().length) {
                    $searchSuggestions.addClass('doing-search');
                }
                
                // Use WooCommerce AJAX endpoint URL
                var ajaxUrl = (typeof wc_add_to_cart_params !== 'undefined') ? wc_add_to_cart_params.wc_ajax_url : fugu_wp_vars.woocommerceAjaxUrl;
                ajaxUrl = ajaxUrl.toString().replace('%%endpoint%%', 'fugu_shop_search');
                ...
                self.suggestionsAjax = $.ajax({
                    ...
                    complete: function() {
                        $searchForm.removeClass('fugu-loader');
		...
		suggestionsShow: function(productsHTML, resultCount, $searchSuggestions) {
            var self = this,
                $productList = $('#fugu-search-suggestions-product-list');
            ...
            $('#fugu-search-suggestions-notice').removeClass().addClass('show ' + noticeTxtClass);
		...
		suggestionsHide: function() {
            ...
            $('#fugu-search-suggestions-product-list').html('');
            $('#fugu-search-suggestions').removeClass();
            $('#fugu-search-suggestions-notice').removeClass();
        ...
        instantSuggestionsLoad: function() {
            ...
            // Use WooCommerce AJAX endpoint URL
            var ajaxUrl = (typeof wc_add_to_cart_params !== 'undefined') ? wc_add_to_cart_params.wc_ajax_url : fugu_wp_vars.woocommerceAjaxUrl;
            ajaxUrl = ajaxUrl.toString().replace('%%endpoint%%', 'fugu_suggestions_product_data');
        ...
        instantSuggestionsGet: function(searchKey) {
            ...
            if (searchResults.length > 0) {
                ...
                self.suggestionsShow(productsHTML, searchResults.length, $('#fugu-search-suggestions'));
            } else {
                self.suggestionsShow('', 0, $('#fugu-search-suggestions'));
            }
        }
