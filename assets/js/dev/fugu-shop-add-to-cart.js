	...
	$.extend($.nmTheme, {
		 ...
		 */
		 atcBind: function() {
			...
			self.$body.on('click', '.single_add_to_cart_button', function(e) {
				...
				if ($thisButton.hasClass('fugu-simple-add-to-cart-button') || $thisButton.hasClass('fugu-variable-add-to-cart-button')) {
					e.preventDefault();
		...
		atcAjaxSubmitForm: function($thisButton, data) {
            ...
            if (typeof wc_add_to_cart_params !== 'undefined') {
                ...
                var ajaxUrl = fugu_wp_vars.woocommerceAjaxUrl;
            }
            ajaxUrl = ajaxUrl.toString().replace('wc-ajax=%%endpoint%%', 'add-to-cart=' + data.product_id + '&fugu-ajax-add-to-cart=1');
			...
			$.ajax({
				...
				success: function(response) {
					var $response = $('<div>' + response + '</div>'), // Wrap the returned HTML string so we can get the replacement elements
						$shopNotices = $response.find('#fugu-shop-notices-wrap'), // Shop notices
					...
					var fragments = {
						'.fugu-menu-cart-count': $response.find('.fugu-menu-cart-count').prop('outerHTML'), // Header menu cart count
						'#fugu-shop-notices-wrap': $shopNotices.prop('outerHTML'),
						//'#fugu-cart-panel': $response.find('#fugu-cart-panel').prop('outerHTML') // Cart panel
					...
					if (self.quickviewOpen) {
						...
						if (hasError) {
							setTimeout(function() {
								// Hide cart panel and overlay
								$('#fugu-cart-panel-overlay').trigger('click');
