	...
	$.extend($.nmTheme, {
		...
		cart_init: function() {
            ...
            self.$body.on('added_to_cart', function() {
                ...
                if ($('#fugu-quickview').is(':visible')) {
                    self.cartTriggerUpdate();
                }
            });
            
            /* Coupon - Bind: Coupon toggle button */
            $('#fugu-coupon-btn').on('click', function(e) {
                e.preventDefault();
                $(this).next('.fugu-coupon').slideToggle(300);
            });
            
            /* Coupon - Bind: Input text change */
            $('.cart-collaterals').on('change keyup paste', '#fugu-coupon-code', function() {
                self.cartCouponFormSetButtonState(this);
            });
            
            /* Coupon: Set button state */
            var $couponInput = $('#fugu-coupon-code');
        ...
        cartCouponFormSetButtonState: function(couponInput) {
            if ($(couponInput).val().length > 1) {
                $('#fugu-apply-coupon-btn').removeAttr('disabled');
            } else {
                $('#fugu-apply-coupon-btn').attr('disabled', 'disabled');
            }
        },
