(function($) {
	...
	$.extend($.nmTheme, {
		...
		filters_init: function() {
			...
			/* Shop filters */
			self.$shopFilterMenu = $('#fugu-shop-filter-menu');
			self.$shopSidebarPopupBtn = $('#fugu-shop-sidebar-popup-button');
            self.shopSidebarLayout = $('#fugu-shop-sidebar').data('sidebar-layout'); // Get the sidebar/filters layout
		...
		shopFiltersBind: function() {
			...
			if (self.filtersEnableAjax) {
                self.$header.on('click.nmHeaderShopAjax', '.shop-ajax-link > a', function(e) {
					...
					$('#fugu-shop-categories-wrap').find('.current-cat').removeClass('current-cat');
			...
			if (self.filtersEnableAjax && self.$pageIncludes.hasClass('shop_categories')) {
                self.$shopWrap.on('click', '#fugu-shop-categories-wrap a', function(e) {
					e.preventDefault();
            ...
            if (self.$shopSidebarPopupBtn.length) {
                self.$shopSidebarPopup = $('#fugu-shop-sidebar-popup');
				...
				$('#fugu-shop-sidebar-popup-reset-button').on('click', function(e) {
					e.preventDefault();
                    self.shopFiltersPopupReset();
				});
                
                /* Bind: Filters popup menu - Close button */
				$('#fugu-shop-sidebar-popup-close-button').on('click', function(e) {
					e.preventDefault();
            ...
            self.$shopWrap.on('click', '#fugu-shop-sidebar .fugu-widget-title', function(e) {
				var $widgetLi = $(this).closest('li');
                
                if ($widgetLi.hasClass('show')) {
                    if (shopWidgetAnimateToggle) {
                        $widgetLi.children('.fugu-shop-widget-col').last().css('height', '');
                    }
                    
                    $widgetLi.removeClass('show');
                } else {
                    var $openWidgetLi = $widgetLi.parent('#fugu-shop-widgets-ul').children('.show');
                    
                    if (shopWidgetAnimateToggle) {
                        var $widgetContentCol = $widgetLi.children('.fugu-shop-widget-col').last(),
                            ...
                            $openWidgetContentCol = $openWidgetLi.children('.fugu-shop-widget-col').last();
			...
			if (self.filtersEnableAjax && self.$pageIncludes.hasClass('shop_filters')) {
				 ...
				 *	- Color
				 */
				self.$shopWrap.on('click', '#fugu-shop-sidebar .fugu_widget a', function(e) {
					e.preventDefault();
					self.shopGetPage($(this).attr('href'));
				});
				
				/* Bind: "Product Categories" widget */
				self.$shopWrap.on('click', '#fugu-shop-sidebar .widget_product_categories a', function(e) {
					e.preventDefault();
					self.shopGetPage($(this).attr('href'));
				});
				
				/* Bind: "Filter Products by Attribute" widget */
				self.$shopWrap.on('click', '#fugu-shop-sidebar .widget_layered_nav a', function(e) {
					e.preventDefault();
					self.shopGetPage($(this).attr('href'));
				});
                 ...
                 */
                self.$document.on('fugu_ajax_shop_update_content', function() {
                    // Code based on inline WooCommerce script on shop-catalog page
                    self.$shopWrap.on('change', '#fugu-shop-sidebar select.woocommerce-widget-layered-nav-dropdown', function() {
                        var $select = $(this),
				...
				self.$shopWrap.on('click', '#fugu-shop-sidebar .widget_layered_nav_filters a', function(e) {
					e.preventDefault();
					self.shopGetPage($(this).attr('href'));
				});
								
				/* Bind: "Product Tag Cloud" widget */
				self.$shopWrap.on('click', '#fugu-shop-sidebar .widget_product_tag_cloud a', function(e) {
					e.preventDefault();
					self.shopGetPage($(this).attr('href'), false, true); // Args: pageUrl, isBackButton, isProductTag
				});
                
                /* Bind: "Filter Products by Rating" widget */
                self.$shopWrap.on('click', '#fugu-shop-sidebar .widget_rating_filter a', function(e) {
					e.preventDefault();
					self.shopGetPage($(this).attr('href'));
				});
                ...
                self.$body.on('price_slider_change', function(event, min, max) {
                    var $priceSliderForm = $('#fugu-shop-sidebar').find('.widget_price_filter').first().find('form'),
			...
			if (self.filtersEnableAjax && self.$pageIncludes.hasClass('shop_filters')) {
                self.$shopWrap.on('click.nmShopIndividualFilterReset', '.fugu-shop-results-bar a:not(.fugu-shop-reset-button)', function(e) {
                    e.preventDefault();
					self.shopGetPage($(this).attr('href'));
                });
            }
		...
		shopFiltersCategoriesToggle: function() {
			var self = this,
                $shopCategories = $('#fugu-shop-categories-wrap'),
		...
		shopFiltersCategoriesReset: function() {
			$('#fugu-shop-categories-wrap').removeClass('fade-in force-show').css('display', '');
		},
        ...
        shopFiltersWidgetHideOpen: function() {
            var $openWidgetLi = $('#fugu-shop-widgets-ul').children('.show');
            if ($openWidgetLi.length) {
                $openWidgetLi.find('.fugu-widget-title').trigger('click'); // Hide open widget
            }
        },
        ...
        shopFiltersPopupReset: function() {
            ...
            if (fugu_wp_vars.shopFiltersPopupAutoClose !== '0') {
                self.shopFiltersPopupHide();
            }
        },
		...
		shopGetPage: function(pageUrl, isBackButton, isProductTag) {
			...
			if (pageUrl) {
                ...
                if (self.shopSidebarLayout == 'popup' && fugu_wp_vars.shopFiltersPopupAutoClose !== '0') {
                    // Hide filters popup
                    self.shopFiltersPopupHide();
                }
				...
				if (self.$body.width() < self.filterPanelHideWidth) {
                    ...
                    if (fugu_wp_vars.shopFiltersMobileAutoClose !== '0') {
		...
		shopUpdateContent: function(ajaxHTML) {
			...
			// Page title - wp_title()
			if (fugu_wp_vars.shopAjaxUpdateTitle) {
				var wpTitle = $ajaxHTML.find('#fugu-wp-title').text();
				if (wpTitle.length) {
			...
			// Extract elements
            var $ajaxBodyClass = $ajaxHTML.find('#fugu-body-class'),
                $ajaxTaxonomyBanner = $ajaxHTML.find('#fugu-shop-taxonomy-header'),
                $ajaxTaxonomyHeading = $ajaxHTML.find('.fugu-shop-taxonomy-heading'),
                $ajaxCategories = $ajaxHTML.find('#fugu-shop-categories-wrap'),
				$ajaxSidebarFilters = $ajaxHTML.find('#fugu-shop-widgets-ul'),
				$ajaxShopBrowseWrap = $ajaxHTML.find('#fugu-shop-browse-wrap');
            ...
            if ($ajaxTaxonomyBanner.length) {
                var $taxonomyBanner = $('#fugu-shop-taxonomy-header');/*,
                    taxonomyBannerURL = $taxonomyBanner.children('.fugu-shop-taxonomy-header-inner').css('background-image'),
                    ajaxTaxonomyBannerURL = $ajaxTaxonomyBanner.children('.fugu-shop-taxonomy-header-inner').css('background-image');*/
                ...
                $('#fugu-shop-taxonomy-header').addClass('hidden');
            }
            
            // Replace: Taxonomy heading
            if ($ajaxTaxonomyHeading.length) {
                $('.fugu-shop-taxonomy-heading').replaceWith($ajaxTaxonomyHeading);
            } else {
                $('.fugu-shop-taxonomy-heading').addClass('hidden');
            }
            
			// Replace: Categories
			if ($ajaxCategories.length) {
				var $shopCategories = $('#fugu-shop-categories-wrap');
            ...
            if ($ajaxSidebarFilters.length) {
                $('#fugu-shop-widgets-ul').replaceWith($ajaxSidebarFilters);
            ...
            self.$document.trigger('fugu_ajax_shop_update_content', $ajaxHTML);
            
			// Get the new shop browse wrap
			self.$shopBrowseWrap = $('#fugu-shop-browse-wrap');
            ...
            if (self.$body.hasClass('fugu-shop-scroll-enabled')) {
                // Smooth-scroll to top
                var to = self.shopScrollToTop();
                setTimeout(function() {
                    // Hide 'loader' overlay (after scroll animation)
