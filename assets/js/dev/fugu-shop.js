	...
	$.extend($.nmTheme, {
		...
		shop_init: function() {
			...
			if (self.isShop) {
				...
				self.shopAjax = false;
				self.scrollOffsetDesktop = parseInt(fugu_wp_vars.shopScrollOffset);
                self.scrollOffsetTablet = parseInt(fugu_wp_vars.shopScrollOffsetTablet);
                self.scrollOffsetMobile = parseInt(fugu_wp_vars.shopScrollOffsetMobile);
                self.infloadScroll = false;
				self.categoryClicked = false;
				self.shopLoaderSpeed = 300;
				self.shopScrollSpeed = 410;
				self.$shopBrowseWrap = $('#fugu-shop-browse-wrap');
				self.imageLazyLoading = (fugu_wp_vars.shopImageLazyLoad != '0') ? true : false;
				if (fugu_wp_vars.shopFiltersAjax != '0') {
					// Check if AJAX should be disabled on mobile devices
					self.filtersEnableAjax = (self.isTouch && fugu_wp_vars.shopFiltersAjax != '1') ? false : true;
				} else {
				 ...
				 */
				$('#fugu-main-menu-ul').children('.shop-link').find('> a').on('click', function(e) {
					e.preventDefault();
                ...
                self.$shopWrap.on('click.nmShopFiltersReset', '#fugu-shop-filters-reset', function(e) {
                    e.preventDefault();
                    self.shopFiltersReset(this);
                });
                
                /* Bind: Results bar - Search/taxonomy reset link */
                self.$shopWrap.on('click.nmShopSearchTaxonomyReset', '#fugu-shop-search-taxonomy-reset', function(e) {
                    e.preventDefault();
			...
			if (typeof wc_add_to_cart_params !== 'undefined' && wc_add_to_cart_params.cart_redirect_after_add !== 'yes') { // Only bind if add-to-cart redirect is disabled
                ...
                self.$body.on('adding_to_cart', function(event, $button, data) {
					...
					if (self.$cartPanel.length) {
						...
						// Show page overlay
                        self.$pageOverlay.addClass('fugu-loader');
                ...
                self.$body.on('added_to_cart', function(event, fragments, cartHash) {
					...
					if (self.$cartPanel.length) {
                        ...
                        self.$pageOverlay.removeClass('fugu-loader');
					}
				});
			} else {
		...
		shopSetMinHeight: function() {
			var self = this,
				footerHeight = $('#fugu-footer').outerHeight(true);
		...
		shopScrollToTop: function(setHeight, jumpTo) {
            ...
            if (self.$window.width() > 399) {
                ...
                var shopPosition = Math.round($('#fugu-shop-products').offset().top - 24 - self.scrollOffset);
            }
		...
		shopShowNotices: function() {
            var self = this;
            self.$body.addClass('fugu-woocommerce-message-show');
            if (self.isShop && fugu_wp_vars.cartPanelHideOnAtcScroll != '0') {
		...
		shopShowLoader: function(disableAnimation) {
			var self = this,
                $shopLoader = $('#fugu-shop-products-overlay');
		...
		shopHideLoader: function(disableAnimation) {
			...
			$shopLoader.removeClass('fugu-loader').addClass('fade-out');
			setTimeout(function() {
				$shopLoader.removeClass('show fade-out').addClass('fugu-loader'); 
		...
		shopSearchTaxonomyReset: function(resetBtn) {
            ...
            if ($resetButton.closest('.fugu-shop-results-bar').hasClass('is-search')) {
                // Search
		...
		singleProductVariationsInit: function($variationsForm) {
            ...
            if (self.shopCustomSelect) {
                ...
                for (var i = 0; i < $selectBoxes.length; i++) {
                    ...
                    if (! $selectBox.parent().hasClass('fugu-select-hidden')) {
            ...
            $variationsWrap.find('.fugu-variation-control').children('li').on('click', function() {
                if (clickThrottle) { return; } // Prevent selection before variation-image has changed
                
                var $this = $(this),
                    $container = $this.parent(),
                    $select = $container.parent().children('.fugu-select-hidden').find('select'),
		...
		singleProductVariationsUpdate: function() {
            ...
            $('.variations_form').find('select').each(function() {
                ...
                if ($select.parent('.fugu-select-hidden').length) {
                    var $controls = $select.closest('.value').children('.fugu-variation-control').children('li'),
                        $control,
                        controlValue,
                        $optionMatch,
                        optionDisabledAttr;
		...
		quickviewIsOpen: function() {
            if ( $('#fugu-quickview').is(':visible') && ! $('#fugu-quickview').hasClass('fade-out') ) {
                return true;
            }
