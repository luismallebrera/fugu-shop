(function($) {
	...
	$.extend($.nmTheme, {
		...
		blog_init: function() {
            ...
            self.$blogList = $('#fugu-blog-list');
            
            
            // Bind: Categories toggle link
			$('#fugu-blog-categories-toggle-link').on('click', function(e) {
				e.preventDefault();
				
				var $thisLink = $(this);
				
				$('#fugu-blog-categories-list').slideToggle(200, function() {
					var $this = $(this);
		...
		blogInfLoadBind: function() {
            ...
            self.$blogPaginationWrap = $('#fugu-blog-pagination');
            self.$blogInfLoadWrap = $('#fugu-blog-infinite-load');
			...
			if (self.$blogInfLoadWrap.length) {
                ...
                if (self.infloadScroll) {
                    ...
                    var pxFromWindowBottomToBottom,
                        ...
                        bufferPx = parseInt(fugu_wp_vars.infloadBufferBlog);
                    ...
                    self.$window.off('resize.nmBlogInfLoad').on('resize.nmBlogInfLoad', function() {
                        ...
                        to = setTimeout(function() {
                            var $infloadControls = $('#fugu-blog-infinite-load'); // Note: Don't cache, element is dynamic
		...
		blogInfLoadGetPage: function() {
			...
			if (nextPageUrl) {
				// Show 'loader'
				self.$blogPaginationWrap.addClass('loading fugu-loader');
                ...
                self.$document.trigger('fugu_blog_infload_before', nextPageUrl);
				...
				self.blogAjax = $.ajax({
					...
					error: function(XMLHttpRequest, textStatus, errorThrown) {
                        // Hide 'loader'
						self.$blogPaginationWrap.removeClass('loading fugu-loader');
                        
						console.log('NM: AJAX error - blogInfLoadGetPage() - ' + errorThrown);
					},
					success: function(response) {
						var $response = $('<div>' + response + '</div>'), // Wrap the returned HTML string in a dummy 'div' element we can get the elements
							$newElements = $response.find('#fugu-blog-list').children();
                        ...
                        if (self.$pageIncludes.hasClass('blog-masonry')) {
                            ...
                            $lastNewImage.on('load', function() {
                                ...
                                self.$document.trigger('fugu_blog_infload_after', $newElements);
		...
		blogInfLoadPrepButton: function($response) {
            var self = this,
                nextPageUrl = $response.find('#fugu-blog-infinite-load').children('a').attr('href');
