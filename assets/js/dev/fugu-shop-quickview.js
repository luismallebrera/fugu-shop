(function($) {
	...
	$.extend($.nmTheme, {
		...
		quickview_init: function() {
			var self = this,
				$qvContainer = $('#fugu-quickview'),
				$qvOverlay = $('<div id="fugu-quickview-overlay" class="mfp-bg fugu-mfp-fade-in"></div>'),
                productId;
            
            
			/* Bind: Quick view links */
			if (fugu_wp_vars.quickviewLinks.link == '1') {
                self.$body.on('click', '.fugu-quickview-btn', function(e) {
                    e.preventDefault();
                    _qvClick(this);
                });
            }
            ...
            if (fugu_wp_vars.quickviewLinks.thumb == '1') {
                self.$body.on('click', '.fugu-shop-loop-thumbnail-link', function(e) {
                    e.preventDefault();
                    _qvClick(this);
                });
            }
            ...
            /* Bind: Title links */
            if (fugu_wp_vars.quickviewLinks.title == '1') {
                self.$body.on('click', '.fugu-shop-loop-title-link', function(e) {
                    e.preventDefault();
                    _qvClick(this);
                });
            }
            ...
            var _qvClick = function(link) {
				...
				if (productId) {
					...
					$qvOverlay.addClass('show mfp-ready fugu-loader');
			...
			var _qvLoadProduct = function() {
				...
				if (typeof wc_add_to_cart_params !== 'undefined') {
					ajaxUrl = wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'fugu_ajax_load_product'); // WooCommerce Ajax endpoint URL (available since 2.4)
				} else {
					ajaxUrl = fugu_wp_vars.ajaxUrl;
					data['action'] = 'fugu_ajax_load_product';
				}
				
				window.fugu_quickview_get_product = $.ajax({
					type: 'POST',
					url: ajaxUrl,
					data: data,
					dataType: 'html',
					...
					beforeSend: function() {
						...
						if (typeof window.fugu_quickview_get_product === 'object') {
							window.fugu_quickview_get_product.abort();
						}
					},
					...
					success: function(data) {
						...
						var $currentContainer = $qvContainer.children('#product-'+productId),
							...
							$imgs = $('#fugu-quickview-slider').find('img'),
                            $lastImg = $imgs.last();
						...
						$lastImg.one('load', function() { // Note: Using ".one()" to make sure the event is only fired once (buggy otherwise)
							...
							self.quantityInputsBindButtons($('#fugu-qv-product-summary'));
			...
			_qvShowModal = function() {
				// Open via API
				$.magnificPopup.open({
                    mainClass: 'fugu-mfp-quickview fugu-mfp-fade-in',
					...
					callbacks: {
						open: function() {
							$qvOverlay.removeClass('fugu-loader'); // Hide 'loader'
                            ...
                            self.$document.trigger('fugu_ajax_quickview_open', productId);
						},
			...
			_qvInitSlider = function() {
				...
				self.$qvSlider = $('#fugu-quickview-slider');
				
				if (self.$qvSlider.children().length > 1) {
					self.qvHasSlider = true;
					
                    var infinite = (fugu_wp_vars.quickViewGalleryInfinite == '1') ? true : false;
