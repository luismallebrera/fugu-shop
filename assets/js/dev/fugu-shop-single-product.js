(function($) {
	...
	$.extend($.nmTheme, {
		...
		singleProduct_init: function() {
            ...
            self.$productWrap            = $('.fugu-single-product');
            ...
            self.galleryThumbnailsSlider = ( fugu_wp_vars.galleryThumbnailsSlider != '0' && self.$productWrap.hasClass('thumbnails-vertical')) ? true : false;
            ...
            /* Accordion */
            self.$productAccordion = $('#fugu-product-accordion');
        ...
        singleProductPinSummaryInit: function() {
            var self = this,
                $productDetailsPinWrap = $('#fugu-summary-pin'),
                offsetHeader = (self.$body.hasClass('header-fixed')) ? Math.round(self.$header.outerHeight()) : 0,
                offset = parseInt(fugu_wp_vars.productPinDetailsOffset) + offsetHeader,
            ...
            $productDetailsPinWrap.pin({
                ...
                containerSelector: '.fugu-single-product-summary-col',
        ...
        singleProductPinSummaryRemove: function() {
            $('#fugu-summary-pin').attr('style', '').removeData('pin');
        },
        ...
        singleProductGalleryInit: function() {
            ...
            // Added in "singleProduct_init" instead: if (self.$galleryImages.length > 1) {
                ...
                if (self.galleryThumbnailsSlider) {
                    ...
                    self.$galleryThumbsContainer.wrapInner('<ol id="fugu-product-gallery-thumbs-inner"></ol>');
                    self.$galleryThumbsWrap = $('#fugu-product-gallery-thumbs-inner');
                }
                ...
                self.$window.on('resize.nmProductGallery', function() {
                    ...
                    timer = setTimeout(function() {
                        ...
                        $('#fugu-variations-form').trigger('click');
                    }, 250);
                });
                ...
                if (self.isTouch) {
                    self.$document.on('touchstart.nmProductGallery', function(e) {
                        if ($(e.target).closest('.woocommerce-product-gallery').length) {
                            self.$galleryContainer.addClass('fugu-touch-hover');
                        } else {
                            self.$galleryContainer.removeClass('fugu-touch-hover');
                        }     
                    });
                }
            //}
		...
		singleProductVariations: function() {
			var self = this,			
                $variationsForm = $('#fugu-variations-form');
            
            $variationsForm = ($variationsForm.length) ? $variationsForm : $('.variations_form'); // Some plugins remove the custom "#fugu-variations-form" id
            
            /* Init variation controls */
            self.singleProductVariationsInit($variationsForm); // Note: Function is placed in the "fugu-shop" file
		...
		singleProductFeaturedVideoInit: function() {
			...
			self.$featuredVideoBtn = $('#fugu-featured-video-link');
			...
			if (self.$featuredVideoBtn.length) {
				...
				self.$featuredVideoBtn.on('click', function(e) {
                    ...
                    // Modal settings
                    var mfpSettings = {
						mainClass: 'fugu-featured-video-popup fugu-mfp-fade-in',
                    ...
                    if (fugu_wp_vars.shopYouTubeRelated == '0') {
                        mfpSettings['iframe'] = {
                            patterns: {
                                youtube: {
        ...
        singleProductAccordionBind: function() {
            ...
            var _toggleCallback = function() {
                ...
                self.$body.trigger('fugu_single_product_accordion_toggle_callback');
            };
            
            /* Toggle selected panel */
            var _togglePanel = function(panel) {
                var $panelHeading = $(panel);
                $panelHeading.parent('.fugu-product-accordion-panel').toggleClass('active');
                $panelHeading.next('.fugu-product-accordion-content').slideToggle(200, function() { _toggleCallback(); });
            };
            
            /* Toggle selected panel and close open panel */
            var _togglePanels = function(panel) {
                var $panelHeading = $(panel),
                    $panel = $panelHeading.parent('.fugu-product-accordion-panel'),
                    $panelContent = $panelHeading.next('.fugu-product-accordion-content');
                ...
                if ($panel.hasClass('open')) {
                    ...
                    if ($panelOpen.length) {
                        var $panelOpenContent = $panelOpen.children('.fugu-product-accordion-content');
            ...
            self.$productAccordion.children('.fugu-product-accordion-panel').children('.fugu-product-accordion-heading').on('click', function(e) {
                e.preventDefault();
                if (fugu_wp_vars.productAccordionCloseOpen != '0') {
                    _togglePanels(this);
                } else {
                    _togglePanel(this);
                }
        ...
        singleProductReviewFormButton: function() {
            $('#fugu-review-form-btn').magnificPopup({
                mainClass: 'fugu-review-form-popup fugu-mfp-fade-in',
                closeMarkup: '<a class="mfp-close fugu-font fugu-font-close2"></a>',
                removalDelay: 180,
                closeBtnInside: true,
                items: {
