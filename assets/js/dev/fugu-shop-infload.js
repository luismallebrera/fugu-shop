(function($) {
	...
	$.extend($.nmTheme, {
		...
		infload_init: function() {
			var self = this,			
				$shopPagination = self.$shopBrowseWrap.children('.fugu-pagination');
            
			// Init
			if ($shopPagination.length && $shopPagination.hasClass('fugu-infload')) {
				// Bind
		...
		shopInfLoadBind: function() {
			var self = this,
                $productsWrap = $('#fugu-shop-products'),
				$infloadControls = self.$shopBrowseWrap.children('.fugu-infload-controls'),					
				nextPageUrl;
			...
			if (self.infloadScroll) {
				...
				var pxFromWindowBottomToBottom,
					...
					bufferPx = parseInt(fugu_wp_vars.infloadBuffer);
				...
				self.$window.off('resize.nmInfLoad').on('resize.nmInfLoad', function() {
					...
					to = setTimeout(function() {
						var $infloadControls = self.$shopBrowseWrap.children('.fugu-infload-controls'); // Note: Don't cache, element is dynamic
				...
				/* Bind: "Load" button */
				$productsWrap.on('click', '.fugu-infload-btn', function(e) {
					e.preventDefault();
					self.shopInfLoadGetPage();
				});
			}
            ...
            $productsWrap.on('click', '.fugu-infload-to-top', function(e) {
                e.preventDefault();
		...
		shopInfLoadGetPage: function() {
			...
			var $nextPageLink = self.$shopBrowseWrap.children('.fugu-infload-link').find('a'),
				$infloadControls = self.$shopBrowseWrap.children('.fugu-infload-controls'),
                $infloadButtonWrap = $infloadControls.children('.fugu-infload-controls-button'),
			...
			if (nextPageUrl) {
                ...
                $infloadButtonWrap.addClass('fugu-loader');
                
                self.$document.trigger('fugu_infload_before', nextPageUrl);
				...
				self.shopAjax = $.ajax({
					...
					complete: function() {
                        ...
                        $infloadButtonWrap.removeClass('fugu-loader');
					},
					success: function(response) {
						var $response = $('<div>' + response + '</div>'), // Wrap the returned HTML string in a dummy 'div' element we can get the elements
							$newElements = $response.children('.fugu-products').children('li');
						...
						self.$shopBrowseWrap.find('.fugu-products:first').append($newElements); // Note: Adding ":first" since a Product shortcode could be added to the category description
						
                        // Preserve/reset scroll position (only needed for Chromium based browsers)
                        if (fugu_wp_vars.infloadPreserveScrollPos != '0' && self.isChromium) {
						...
						nextPageUrl = $response.find('.fugu-infload-link').children('a').attr('href');
						if (nextPageUrl) {
                        ...
                        self.$document.trigger('fugu_infload_after', $newElements);
					}
				});
			} else {
				if (self.infloadScroll) {
