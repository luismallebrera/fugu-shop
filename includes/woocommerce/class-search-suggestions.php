...
class FUGU_Search_Suggestions {
	...
	public function __construct() {
        ...
        if ( $fugu_theme_options['shop_search_suggestions_instant'] ) {
            // Action: WooCommerce AJAX endpoint - Suggestions product data
            add_action( 'wc_ajax_fugu_suggestions_product_data', array( $this, 'instant_suggestions_get_product_data' ) );
            ...
            // Transients
            //$this->transient_enabled = ( $fugu_theme_options['shop_search_suggestions_cache'] ) ? true : false;
            //$this->transient_duration = intval( $fugu_theme_options['shop_search_suggestions_cache_expiration'] ) * HOUR_IN_SECONDS; // Hours in seconds
            $this->transient_enabled = apply_filters( 'fugu_search_suggestions_cache', false );
            $this->transient_duration = intval( apply_filters( 'fugu_search_suggestions_cache_expiration', 12 ) ) * HOUR_IN_SECONDS; // Hours in seconds

            // Maximum results
            $this->max_results = $fugu_globals['shop_search_suggestions_max_results'];
            
            // Action: WooCommerce AJAX endpoint - Search
            add_action( 'wc_ajax_fugu_shop_search', array( $this, 'shop_search' ) );
        }
    }
	...
	public function shop_search() {
        ...
        $transient_name = 'fugu_search_suggestions_' . $search_keyword;
        $suggestions = '';
        ...
        if ( ! $this->transient_enabled || false === ( $suggestions = get_transient( $transient_name ) ) ) {
            ...
            $args = apply_filters( 'fugu_search_suggestions_query_args', array(
                's'                   => $search_keyword,
                'post_type'           => 'product',
                'post_status'         => 'publish',
	...
	public function instant_suggestions_get_product_data() {
        $generate_cache = intval( apply_filters( 'fugu_instant_suggestions_generate_cache', get_option( 'fugu_instant_suggestions_generate_cache', 1 ) ) );
        
        if ( $generate_cache == 1 ) {
            
            $args = apply_filters( 'fugu_instant_suggestions_query_args', array(
                'status'        => 'publish',
                'visibility'    => 'search',
                'limit'         => -1,
                //'orderby'       => 'modified',
            ...
            if ( ! empty( $products ) ) {
                ...
                $i = 0;
                $thumbnail_size = apply_filters( 'fugu_instant_suggestions_image_size', 'woocommerce_thumbnail' );
                $search_categories = apply_filters( 'fugu_instant_suggestions_search_categories', false );
                $search_tags = apply_filters( 'fugu_instant_suggestions_search_tags', false );
                $search_sku_option = ( $fugu_theme_options['shop_search_by_sku'] !== 0 ) ? true : false;
                $search_sku = apply_filters( 'fugu_instant_suggestions_search_sku', $search_sku_option );
                ...
                foreach( $products as $product ) {
                    ...
                    $products_array[$i] = array( // Note: Don't set product-ID as array key here or product order will change (JSON object automatically sorts its items by highest to lowest key value)
                        ...
                        'product_html'  => '<li>' . apply_filters( 'fugu_instant_suggestions_product_html', $product_html, $product ) . '</li>'
                    );
            ...
            // Update cache
            update_option( 'fugu_instant_suggestions_product_data', $products_json, true );
            
            // Set cache flag
            update_option( 'fugu_instant_suggestions_generate_cache', 0, true );
        } else {
            // Get from cache
            $products_json = get_option( 'fugu_instant_suggestions_product_data', '{}' );
        }
	...
	public function instant_suggestions_set_cache_flag() {
        update_option( 'fugu_instant_suggestions_generate_cache', 1, true );
    }

}

$fugu_search_suggestions = new FUGU_Search_Suggestions();
