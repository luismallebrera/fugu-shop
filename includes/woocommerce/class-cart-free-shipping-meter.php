...
class FUGU_Cart_Free_Shipping_Meter {
    ...
    public function free_shipping_meter() {
        ...
        /*if ( apply_filters( 'fugu_cfsm_ignore_discounts', $free_shipping_ignore_discounts !== 'no' ) ) {
            $discount     = 0;
            $discount_tax = 0;
        }*/
        ...
        if ( empty( $is_qualified ) ) {
            ...
            $message            = $fugu_theme_options['cart_shipping_meter_message'];
            $message_percent    = round( $percent, 0 ) . '%';
        } else {
            $wrapper_class      = 'is-free';
            $remaining          = 0;
            $percent            = 100;
            $message            = $fugu_theme_options['cart_shipping_meter_message_qualified'];
            $message_percent    = '<i class="fugu-font-check-alt"></i>';
        }
        
        $message = $this->placeholders( $message, $remaining, $free_shipping_min_amount );
        ?>
        <div class="fugu-cart-shipping-meter <?php echo esc_attr( $wrapper_class ); ?>">
            <div class="fugu-cart-shipping-meter-top">
                <strong><?php echo esc_html( $message ); ?></strong>
                <span><?php echo $message_percent; ?></span>
            </div>
            <div class="fugu-cart-shipping-meter-bar">
                <div class="fugu-cart-shipping-meter-bar-progress" data-progress="<?php echo esc_attr( $percent ); ?>"></div>
            </div>
        </div>
        <?php
    }
    ...
    public function get_free_shipping() {
        ...
        return apply_filters( 'fugu_cfsm_get_free_shipping', $free_shipping );
    }
...
/**
 * Init FUGU_Cart_Free_Shipping_Meter class
 */
add_action( 'init', function() {
    global $fugu_cart_free_shipping_meter;
    $fugu_cart_free_shipping_meter = new FUGU_Cart_Free_Shipping_Meter();
} );

/**
 * Function: Display free shipping meter
 */
/*function fugu_cart_free_shipping_meter() {
    global $fugu_cart_free_shipping_meter;
    $fugu_cart_free_shipping_meter->free_shipping_meter();
}*/
