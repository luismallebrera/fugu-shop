...
class FUGU_Product_Attributes {
	...
	public function __construct() {
        ...
        $this->pa_types = $fugu_globals['pa_variation_controls'];
    ...
    public function pa_assets( $hook ) {
        ...
        wp_enqueue_script( 'fugu-wp-color-picker', FUGU_URI . '/assets/js/fugu-wp-attributes-color-picker-init.js', array( 'jquery' ), false );
    ...
    public function pa_term_add_form_fields( $taxonomy ) {
        $attr = fugu_woocommerce_get_taxonomy_attribute( $taxonomy );
        $type = $attr->attribute_type;
        
        // Field: Color
        if ( $type == 'color' ) :
            ?>
            <div class="form-field term-fugu_pa_color-wrap">
                <label for="fugu_pa_color"><?php esc_html_e( 'Color', 'fugu-framework-admin' ); ?></label>
                <input type="text" id="fugu_pa_color" name="fugu_pa_color" class="fugu_pa_color-picker" value="" size="40">
            </div>
            <?php
        ...
        elseif ( $type == 'image' ) :
            ?>
            <div class="form-field term-fugu_pa_image-wrap">
                <label><?php esc_html_e( 'Image', 'woocommerce' ); ?></label>
                <div id="fugu_pa_image_thumbnail" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( wc_placeholder_img_src() ); ?>" width="60px" height="60px" /></div>
                <div style="line-height: 60px;">
                    <input type="hidden" id="fugu_pa_image_thumbnail_id" name="fugu_pa_image_thumbnail_id" />
                ...
                <script type="text/javascript">
                    ...
                    if (! jQuery('#fugu_pa_image_thumbnail_id').val()) { jQuery('.fugu-remove_image_button').hide(); }
                    ...
                    jQuery(document).on('click', '.fugu-upload_image_button', function(event) {
                        ...
                        file_frame.on('select', function() {
                            ...
                            jQuery('#fugu_pa_image_thumbnail_id').val(attachment.id);
                            jQuery('#fugu_pa_image_thumbnail').find('img').attr('src', attachment_thumbnail.url);
                    ...
                    jQuery(document).on('click', '.fugu-remove_image_button', function() {
                        jQuery('#fugu_pa_image_thumbnail').find('img').attr('src', '<?php echo esc_js( wc_placeholder_img_src() ); ?>');
                        jQuery('#fugu_pa_image_thumbnail_id').val('');
    ...
    public function pa_term_edit_form_fields( $term, $taxonomy ) {
        ...
        if ( $attr->attribute_type == 'color' ) :
            ...
            if ( $id ) {
                $saved_colors = get_option( 'fugu_pa_colors' );
                $color = ( isset( $saved_colors[$id] ) ) ? $saved_colors[$id] : '';
            }
            ?>
            <tr class="form-field term-fugu_pa_color-wrap">
                <th scope="row">
                    <label for="fugu_pa_color"><?php esc_html_e( 'Color', 'fugu-framework-admin' ); ?></label>
                </th>
                <td>
                    <input type="text" id="fugu_pa_color" name="fugu_pa_color" class="fugu_pa_color-picker" value="<?php echo esc_attr( $color ); ?>" size="40">
                </td>
            </tr>
            <?php
        ...
        elseif ( $attr->attribute_type == 'image' ) :
            $thumbnail_id = absint( get_term_meta( $term->term_id, 'fugu_pa_image_thumbnail_id', true ) );
            ...
            ?>
            <tr class="form-field term-fugu_pa_image-wrap">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Image', 'woocommerce' ); ?></label></th>
                <td>
                    <div id="fugu_pa_image_thumbnail" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( $image ); ?>" width="60px" height="60px" /></div>
                    <div style="line-height: 60px;">
                        <input type="hidden" id="fugu_pa_image_thumbnail_id" name="fugu_pa_image_thumbnail_id" value="<?php echo esc_attr( $thumbnail_id ); ?>" />
                        <button type="button" class="fugu-upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'woocommerce' ); ?></button>
                        <button type="button" class="fugu-remove_image_button button"><?php esc_html_e( 'Remove image', 'woocommerce' ); ?></button>
                    </div>
                    <script type="text/javascript">
                        // Only show the "remove image" button when needed
                        if ('0' === jQuery('#fugu_pa_image_thumbnail_id').val()) { jQuery('.fugu-remove_image_button').hide(); }
    ...
    public function pa_term_save( $term_id ) {
        // Field: Color
        if ( isset( $_POST['fugu_pa_color'] ) ) {
            $color = sanitize_text_field( $_POST['fugu_pa_color'] );
            $saved_colors = get_option( 'fugu_pa_colors' );
            ...
            update_option( 'fugu_pa_colors', $saved_colors );
        }
        
        // Field: Image
        if ( isset( $_POST['fugu_pa_image_thumbnail_id'] ) ) {
			update_term_meta( $term_id, 'fugu_pa_image_thumbnail_id', absint( $_POST['fugu_pa_image_thumbnail_id'] ) );
		}
    }

}

$FUGU_Product_Attributes = new FUGU_Product_Attributes();
