...
class FUGU_Product_Data {
    ...
    public function product_attribute_settings( $attribute, $i ) {
        global $post, $fugu_globals;
        
        // Make sure the attribute is for a variation
        if ( isset( $attribute['variation'] ) && $attribute['variation'] == true ) {
            $attr = fugu_woocommerce_get_taxonomy_attribute( $attribute['name'] ); // "fugu_woocommerce_get_taxonomy_attribute()" is in "../savoy/includes/woocommerce/woocommerce-attribute-functions.php"
            $custom_attr_types = array_keys( $fugu_globals['pa_variation_controls'] );
            ...
            if ( $attr && in_array( $attr->attribute_type, $custom_attr_types ) ) {
                ...
                $saved_data = get_post_meta( $product_id, 'fugu_attribute_catalog_visibility', true );
                $checked = ( $saved_data && isset( $saved_data[$attribute['name']] ) ) ? true : false;
                ?>
                <tr>
                    <td>
                        <label><input type="checkbox" class="checkbox" <?php checked( $checked, true ); ?> name="fugu_attribute_catalog_visibility[<?php echo esc_attr( $attribute['name'] ); ?>]" value="1" /> <?php esc_html_e( 'Show in product catalog', 'fugu-framework-admin' ); ?></label>
                    </td>
                </tr>
                <?php
            }
    ...
    public function product_attribute_settings_save( $product_id = null ) {
        ...
        if ( isset( $_POST['data'], $_POST['post_id'] ) ) {
            ...
            if ( isset( $data['fugu_attribute_catalog_visibility'] ) ) {
                $saved_data = $data['fugu_attribute_catalog_visibility'];
            }
            
            if ( $saved_data ) {
                update_post_meta( $product_id, 'fugu_attribute_catalog_visibility', $saved_data );
            } else {
                delete_post_meta( $product_id, 'fugu_attribute_catalog_visibility' );
            }
        }
        // Static save
        else if ( ! wp_doing_ajax() ) { // Make sure this isn't an AJAX request (like when saving Variations via AJAX)
            if ( isset( $_POST['fugu_attribute_catalog_visibility'] ) ) {
                $saved_data = $_POST['fugu_attribute_catalog_visibility'];
            }
...
$FUGU_Product_Data = new FUGU_Product_Data();
