...
if ( $fugu_theme_options['shop_search_by_titles'] ) {
    /*
     *	Search: Limit to product titles
     *
     *	https://wordpress.org/plugins/woocommerce-filter-search/
     */
    // Note: Using &$wp_query might be causing a PHP warning: function fugu_woocommerce_search_by_title_only( $search, &$wp_query ) {
    function fugu_woocommerce_search_by_title_only( $search, $wp_query ) {
        global $wpdb;
    ...
    add_filter( 'posts_search', 'fugu_woocommerce_search_by_title_only', 500, 2 );
     ...
     *	https://wordpress.org/plugins/woocommerce-filter-search/
     */
    function fugu_woocommerce_disable_excerpt_search( $where = '' ) {
        global $wp_the_query;
    ...
    add_filter( 'posts_where', 'fugu_woocommerce_disable_excerpt_search', 15 );
}
