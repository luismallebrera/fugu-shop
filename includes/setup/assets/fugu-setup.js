    ...
    FUGU_Setup.prototype = {
        ...
        init: function() {
            ...
            if (pageBuilderSelectionQS) {
                $('.fugu-setup-steps li.step-plugins').addClass('active');
                self.stepSetActiveBreadcrumb();
            } else {
                $('.fugu-setup-steps li.step:first-child').addClass('active');
            }

            // Bind: Setup buttons
            $('.fugu-setup-button').on('click', function(e) {
                e.preventDefault();
                self.setupNoticeHide();
                $('.fugu-setup-view').addClass('loading');
         ...
         */
        setupNoticeHide() {
            $('#fugu-setup-notice').addClass('hide');
        },
        ...
        stepShowNext: function(btn) {
            ...
            setTimeout(function() {
                // Set active step
                var $stepActive = $('.fugu-setup-steps').children('.active');
        ...
        stepHideLoader: function() {
            $('.fugu-setup-view').removeClass('loading');
        },
        ...
        stepSetActiveBreadcrumb: function($stepActive) {
            var $breadcrumbs = $('.fugu-setup-breadcrumbs'),
                $stepActive = $('.fugu-setup-steps').children('.active'),
        ...
        pageBuilderSelect: function() {
            var self = this,
                selection = $('#fugu-setup-page-builder-select').find('input:checked').val();
            
            $.ajax({
                type: 'POST',
                url: fugu_setup_params.ajaxurl,
                data: {
                    action: 'page_builder_save_selection',
                    wpnonce: fugu_setup_params.wpnonce,
                    selection: selection
                },
        ...
        pluginsInstall: function() {
            ...
            function _processCurrent() {
                if (current_item) {
                    ...
                    jQuery.post(fugu_setup_params.ajaxurl, {
            ...
            function _findNext() {
                ...
                var $li = $('.fugu-setup-tasks-plugins li');
                $li.each(function() {
        ...
        contentInstall: function() {
            ...
            _ajaxInstallSettings = function() {
                $.ajax({
                    ...
                    beforeSend: function() {
                        _setProgressMessage('.fugu-setup-task-settings', 'Configuring Settings...');
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        self.setupNoticeShow('_ajaxInstallSettings(): '+errorThrown, 'error');
                        self.stepHideLoader();
                        _setProgressMessage('.fugu-setup-task-settings', '<em class="error">Failed, please try again</em>');
                    },
                    success: function(response) {
                        _setProgressMessage('.fugu-setup-task-settings', 'Done', true);
            ...
            _ajaxInstallWidgets = function() {
                $.ajax({
                    ...
                    beforeSend: function() {
                        _setProgressMessage('.fugu-setup-task-content', 'Importing Widgets...');
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        self.setupNoticeShow('_ajaxInstallWidgets(): '+errorThrown, 'error');
                        self.stepHideLoader();
                        _setProgressMessage('.fugu-setup-task-content', '<em class="error">Failed, please try again</em>');
                    },
                    success: function(response) {
                        _setProgressMessage('.fugu-setup-task-content', 'Done', true);
            ...
            _ajaxInstallContent = function() {
                $.ajax({
                    ...
                    beforeSend: function() {
                        _setProgressMessage('.fugu-setup-task-content', 'Importing Content...');
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    success: function(response) {
                        _setProgressMessage('.fugu-setup-task-content', 'Done');
            ...
            _ajaxInstallWooCommerceTaxonomies = function() {
                $.ajax({
                    ...
                    beforeSend: function() {
                        _setProgressMessage('.fugu-setup-task-content', 'Installing WooCommerce Taxonomies...');
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    success: function(response) {
                        ...
                        if (response == 'woocommerce na') {
                            ...
                            _setProgressMessage('.fugu-setup-task-content', '<em class="error">WooCommerce plugin not activated, please try again</em>');
