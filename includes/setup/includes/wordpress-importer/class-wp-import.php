...
class FUGU_WP_Import extends WP_Importer {
// /NM
...
<?php
	...
	function process_attachment( $post, $url ) {
        ...
        if ( defined( 'FUGU_IMPORT_LOCAL_IMAGES' ) && FUGU_IMPORT_LOCAL_IMAGES ) {
            $file_name = basename( $url );
            $url = FUGU_IMPORT_URI . '/setup/content/images/' . $file_name;
        } else {
            if ( preg_match( '|^/[\w\W]+$|', $url ) )
                $url = rtrim( $this->base_url, '/' ) . $url;
        }
