	...
	$(function() { // Doc ready
        ...
        if (wp && wp.media) {
            ...
            var thumbnailAddInputTag = function($fieldWrap, val) {
                ...
                $fieldWrap.find('label').append('<input type="hidden" name="fugu-menu-item-thumbnail['+menuItemId+']" value="'+val+'">');
            }
            
            /* Field: Thumbnail - Bind "Set thumbnail" link */
            $('#menu-to-edit').on('click', '.fugu-menu-item-set-thumbnail', function(e) {
                e.preventDefault();
                
                $fieldWrap = $(this).closest('.fugu-field-thumbnail');
                $fieldImg = $fieldWrap.find('img');
                $fieldInput = $fieldWrap.find('input');
                $fieldAddLink = $fieldWrap.find('.fugu-menu-item-set-thumbnail');
                $fieldRemoveLink = $fieldWrap.find('.fugu-menu-item-remove-thumbnail');
            ...
            $('#menu-to-edit').on('click', '.fugu-menu-item-remove-thumbnail', function(e) {
                e.preventDefault();
