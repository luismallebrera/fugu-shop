...
if ( strlen( $max_width ) > 0 ) {
    //$type = 'boxed fugu-row-max-width'; // Set type to "boxed" if max-width is set
    $type = $type . ' fugu-row-max-width';
...
// Row class start
$row_class = 'fugu-row fugu-row-' . $type . ' vc_row wpb_row';

// Disable Row
if ( 'yes' === $disable_element ) {
    if ( vc_is_page_editable() ) {
        $row_class .= ' fugu-row-hidden';
	} else {
		return '';
	}
}
...
if ( ! empty( $full_height ) ) {
    fugu_add_page_include( 'row-full-height' );

    $row_class .= ' fugu-row-full-height';
}

// Row flexbox class: Equal height
if ( ! empty( $equal_height ) ) {
    $row_flex_class .= ' fugu-row-equal-height';
}
// Row flexbox class: Content placement
if ( ! empty( $content_placement ) ) {
    $row_flex_class .= ' fugu-row-col-' . $content_placement;
}
// Row flexbox class
if ( ! empty( $row_flex_class ) ) {
    $row_class .= ' fugu-row-flex' . $row_flex_class;
}
...
if ( ! empty( $video_bg ) && ! empty( $video_bg_url ) ) {
    ...
    fugu_add_page_include( 'video-background' );

    // Enqueue YouTube JavaScript API
    wp_enqueue_script( 'fugu_youtube_iframe_api_js', 'https://www.youtube.com/iframe_api', array(), FUGU_THEME_VERSION, true );

    $row_class .= ' fugu-row-video';
    $wrapper_atts[] = 'data-video-url="' . esc_url( $video_bg_url ) . '"';
}    

// Parallax		
if ( ! empty( $parallax ) ) {
    $row_class .= ' fugu-row-parallax fugu-row-parallax-' . $parallax;
