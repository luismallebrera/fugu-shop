...
$class = 'fugu-divider ' . $title_align . $custom_class;

$wrapper_attributes[] = 'class="' . esc_attr( $class ) . '"';

$title = ( strlen( $title ) > 0 ) ? '<' . $title_tag . ' class="fugu-divider-title ' . esc_attr( $title_size ) . '">' . $title . '</' . $title_tag . '>' : '';
...
$output = '
    <div ' . implode( ' ', $wrapper_attributes ) . '>' . 
        $title . '
        <div class="fugu-divider-line" style="' . esc_attr( $divider_style ) . '"></div>
    </div>';
