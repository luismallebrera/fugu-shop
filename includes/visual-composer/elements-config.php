<?php
    ...
    vc_add_param( 'vc_row', array(
		'type' 			=> 'dropdown',
		'heading' 		=> esc_html__( 'Row Type', 'fugu-framework-admin' ),
		'param_name' 	=> 'type',
		'description'	=> esc_html__( 'Select row layout.', 'fugu-framework-admin' ),
		'weight'		=> 1,
		'value' 		=> array(
			'Full'				=> 'full',
	...
	vc_add_param( 'vc_row', array(
		'type' 			=> 'textfield',
		'heading' 		=> esc_html__( 'Maximum Width', 'fugu-framework-admin' ),
		'param_name' 	=> 'max_width',
		'value' 		=> '',
		'description'	=> esc_html__( 'Optional: Enter a maximum width (numbers only).', 'fugu-framework-admin' ),
		'weight'		=> 1
	) );
	vc_add_param( 'vc_row', array(
		'type' 			=> 'textfield',
		'heading' 		=> esc_html__( 'Minimum Height', 'fugu-framework-admin' ),
		'param_name' 	=> 'min_height',
		'value' 		=> '',
		'description'	=> esc_html__( 'Optional: Enter a minimum height (numbers only).', 'fugu-framework-admin' ),
		'weight'		=> 1
	) );
	// Modify "vc_row - parallax" param (instead of removing param and adding new)
	function fugu_vc_row_param_parallax() {
		// Get param values
		$param = WPBMap::getParam( 'vc_row', 'parallax' );
		
		// Replace param values
		$param['value'] = array(
			esc_html__( 'None', 'fugu-framework-admin' ) => '',
			esc_html__( 'Static (fixed background)', 'fugu-framework-admin' ) => 'static'
		);
		
		// Finally "mutate" param with new values
		vc_update_shortcode_param( 'vc_row', $param );
	}
	add_action( 'vc_after_init', 'fugu_vc_row_param_parallax' );
    ...
    vc_add_param( 'vc_separator', array(
		'type' 			=> 'textfield',
		'heading' 		=> esc_html__( 'Title', 'fugu-framework-admin' ),
		'param_name' 	=> 'title',
		'holder' 		=> 'div',
		'value' 		=> '',
		'description'	=> esc_html__( 'Separator title.', 'fugu-framework-admin' ),
		'weight'		=> 1
	) );
	vc_add_param( 'vc_separator', array(
		'type' 			=> 'dropdown',
		'heading' 		=> esc_html__( 'Title Size', 'fugu-framework-admin' ),
		'param_name' 	=> 'title_size',
		'description'	=> esc_html__( 'Select title size.', 'fugu-framework-admin' ),
		'value' 		=> array(
			'Large' 	=> 'large',
			'Medium'	=> 'medium',
			'Small' 	=> 'small',
	...
	vc_add_param( 'vc_separator', array(
		'type' 			=> 'dropdown',
		'heading' 		=> esc_html__( 'Title position', 'fugu-framework-admin' ),
		'param_name'	=> 'title_align',
		'value' 		=> array(
			esc_html__( 'Align center', 'fugu-framework-admin' )	=> 'separator_align_center',
			esc_html__( 'Align left', 'fugu-framework-admin' )	=> 'separator_align_left',
			esc_html__( 'Align right', 'fugu-framework-admin' )	=> "separator_align_right"
		),
		'description'	=> esc_html__( 'Select title location.', 'fugu-framework-admin' ),
		'weight'		=> 1
	) );
    vc_add_param( 'vc_separator', array(
        'type' 			=> 'dropdown',
        ...
        'description'	=> esc_html__( 'Select title HTML tag.', 'fugu-framework-admin' ),
        'value' 		=> array(
            'h1'   => 'h1',
            'h2'   => 'h2',
            'h3'   => 'h3',
	...
	vc_add_param( 'vc_separator', array(
		'type' 			=> 'colorpicker',
		'heading' 		=> esc_html__( 'Custom Border Color', 'fugu-framework-admin' ),
		'param_name' 	=> 'accent_color',
		'description'	=> esc_html__( 'Select border color for your element.', 'fugu-framework-admin' ),
		'weight'		=> 1
	) );
	...
	vc_add_param( 'vc_message', array(
		'type' 			=> 'dropdown',
		'heading' 		=> esc_html__( 'Message Box Presets', 'fugu-framework-admin' ),
		'param_name'	=> 'color',
		'value' 		=> array(
			'Information'	=> 'info',
			'Warning'		=> 'warning',
		...
		'description' => esc_html__( 'Select predefined message box design.', 'fugu-framework-admin' ),
		'weight'		=> 1
	) );
    ...
    if ( ! $fugu_globals['vcomp_stock'] ) {
        ...
        function fugu_vc_single_image_onclick() {
            // Get param values
            $param = WPBMap::getParam( 'vc_single_image', 'onclick' );
            // Replace param values
            $param['value'] = array(
                esc_html__( 'None', 'fugu-framework-admin' ) => '',
                esc_html__( 'Link to large image', 'fugu-framework-admin' ) => 'img_link_large',
                //esc_html__( 'Open prettyPhoto', 'fugu-framework-admin' ) => 'link_image',
                esc_html__( 'Open custom link', 'fugu-framework-admin' ) => 'custom_link',
                //esc_html__( 'Zoom', 'fugu-framework-admin' ) => 'zoom',
            );
            // Finally "mutate" param with new values
            vc_update_shortcode_param( 'vc_single_image', $param );
        }
        add_action( 'vc_after_init', 'fugu_vc_single_image_onclick' );
    }
